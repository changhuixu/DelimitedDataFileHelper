...
internal class DelimitedFileWriter(string delimiter = "\t")
{
    /// <summary>
    ...
    protected virtual string GetHeader(PropertyInfo[] objProperties)
    {
        ...
        {
            result.Append(Escape(objProperty.Name)).Append(delimiter);
        }
    ...
    protected virtual string WriteRows<T>(IEnumerable<T> data, PropertyInfo[] objProperties)
    {
        ...
        foreach (var obj in data)
        {
            foreach (var objProperty in objProperties)
            {
                result.Append(Escape(objProperty.GetValue(obj))).Append(delimiter);
            }
            result.Length -= delimiter.Length;
            result.AppendLine();
        }
